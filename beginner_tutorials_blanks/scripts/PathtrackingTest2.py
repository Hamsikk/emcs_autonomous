...
class HybridController:
    ...
    def pure_pursuit_control(self, point1, point2):
        ...
        if not self.is_look_forward_point:
            ...
            # 장애물 거리가 7 미터 이하일 경우 속도를 1.0으로 설정
            if self.obstacle_distance <= 7:
                self.current_velocity = 1.0
            else:
                # 장애물 거리가 7 미터 이상일 경우 속도를 +0.1 (최대 12)
                self.current_velocity = min(self.current_velocity + 0.3, 12.0)
    ...
    def stanley_control(self, point2):
        ...
        if self.obstacle_distance <= 7:
            ...
            # 장애물 거리가 7 미터 이상일 경우 속도를 +0.1 (최대 12)
            self.current_velocity = min(self.current_velocity + 0.1, 12.0)
